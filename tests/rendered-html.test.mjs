...
test("keeps the application UI-only", async () => {
  ...
  assert.match(page, /className=\{`hamburger-trigger \$\{open \? "open" : ""\}`\}[\s\S]*aria-expanded=\{open\}[\s\S]*aria-controls=\{menuId\}/);
  assert.match(page, /className="page-menu"[\s\S]*PAGE_NAVIGATION\.map/);
  assert.doesNotMatch(styles, /\.view-navigation/);
  assert.match(styles, /\.page-menu\s*\{[^}]*position:\s*absolute;[^}]*right:\s*0;/s);
