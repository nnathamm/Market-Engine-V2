...
test("keeps the application UI-only", async () => {
  ...
  assert.match(page, /className=\{activeView === "notifications" \? "active" : ""\}[\s\S]*Notification Settings/);
  ...
  assert.match(page, /const PAGE_NAVIGATION:[\s\S]*Create Signal[\s\S]*View Signals[\s\S]*Order Flow[\s\S]*Notifications/);
  ...
  assert.match(page, /Master ADMIN[\s\S]*Executive control account[\s\S]*Full site configuration control[\s\S]*Override lower-tier account permissions[\s\S]*Modify, suspend, or delete accounts/);
  assert.match(page, /master-profile-menu[\s\S]*Notification Settings[\s\S]*Manage Accounts/);
  ...
  assert.match(styles, /\.master-profile-menu\s*\{[^}]*position:\s*absolute;[^}]*right:\s*0;/s);
